...
const IVMenuPage = () => {
  ...
  return <div className="min-h-screen bg-background">
            ...
            <div className="max-w-6xl mx-auto">
              <Tabs defaultValue="nutrients" className="w-full">
                ...
                <TabsContent value="nutrients" className="space-y-8">
                  ...
                  <div className="md:hidden">
                    <Accordion type="single" collapsible className="w-full">
                      {nutrients.map((nutrient, index) => <AccordionItem key={index} value={`nutrient-${index}`} id={`nutrient-${nutrient.category.toLowerCase().replace(/\s+/g, '-')}`}>
                          ...
                          <AccordionContent>
                            <div className="space-y-4 pt-4">
                              ...
                              <div className="space-y-2">
                                {nutrient.items.map((item, itemIndex) => <div key={itemIndex} className="text-sm flex items-center">
                  ...
                  <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {nutrients.map((nutrient, index) => <Card key={index} className="border-primary/20 hover:shadow-lg transition-shadow" id={`nutrient-${nutrient.category.toLowerCase().replace(/\s+/g, '-')}`}>
                        <CardHeader>
                        ...
                        <CardContent>
                          <div className="space-y-1">
                            {nutrient.items.map((item, itemIndex) => <div key={itemIndex} className="text-sm text-muted-foreground flex items-center">
                                <div className="h-1 w-1 bg-primary rounded-full mr-2"></div>
                                {item}
                              </div>)}
                          </div>
                ...
                <TabsContent value="packages" className="space-y-8">
                  ...
                  <div className="md:hidden">
                    <Accordion type="single" collapsible className="w-full">
                      {ivPackages.map((pkg, index) => <AccordionItem key={index} value={`package-${index}`} id={`package-${pkg.name.toLowerCase().replace(/\s+/g, '-').replace(/"/g, '')}`}>
                          <AccordionTrigger className="text-left">
                            <div>
                            ...
                            <div className="space-y-4 pt-4">
                              <div>
                                ...
                                <ul className="space-y-1">
                                  {pkg.nutrients.map((nutrient, nutrientIndex) => <li key={nutrientIndex} className="text-sm text-muted-foreground flex items-center">
                                      <div className="h-1 w-1 bg-primary rounded-full mr-2"></div>
                                      {nutrient}
                                    </li>)}
                                </ul>
                                {pkg.specialNote && <div className="mt-3 p-2 bg-primary/5 rounded-lg">
                  ...
                  <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {ivPackages.map((pkg, index) => <Card key={index} className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-primary/20" id={`package-${pkg.name.toLowerCase().replace(/\s+/g, '-').replace(/"/g, '')}`}>
                        <CardHeader>
      ...
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
        <Button variant="medical" size="lg" onClick={() => navigate('/contact')} className="shadow-lg px-8 py-4 text-lg">
          Book a Session
        </Button>
      </div>
